...
const MUSE_WEBSITE = 'http://47.93.187.177:3404/';
let saved = {};
...
function composer(compact = false) {
  ...
  return `<form id="composer-form" class="composer" aria-label="创作对话">
    ...
    </form><div class="composer-caption"><span>${brandMark()} Muse ${memories ? `· ${memories} 条已保存品味` : '· 先理解，再创作'}</span><span>${state.connection.configured ? (state.type === 'image' && !state.connection.imageGeneration ? '当前支持 SVG 矢量插画' : '需求与参考将发送给已配置的模型') : connectionLabel()}</span></div>`;
}
...
function gallery() {
  return `<main id="main" class="content gallery-full"><div class="page-heading"><div><div class="eyebrow">THE MUSE COLLECTION / 精选样板</div><h1>让一个想法，打开另一个。</h1><p>从 ${samples.length} 套完整作品出发，把喜欢的表达，变成自己的创作。</p><div class="gallery-heading-note"><i aria-hidden="true"></i>Muse 6.0 设计 · 可直接预览、导出 · 支持继续创作</div></div></div>${filters()}<div class="gallery-grid">${samples.filter(s => state.filter === 'all' || s.type === state.filter).map(sampleCard).join('')}</div>${footer()}</main>`;
}
...
async function openSample(id) {
  ...
  let p = { id: crypto.randomUUID(), sampleId: sampleProjectKey(id), type: sample.type, title: sample.title, updated: Date.now(), messages: [{ role: 'user', text: sample.prompt }, { role: 'assistant', text: `已打开「${sample.title}」，包含${sample.detail}。你可以直接预览与导出，也可以告诉我想替换的品牌、内容或风格，在这份样板上继续创作。`, version: 0 }], versions: [a] };
...
async function submitPrompt() {
  ...
  const token = { input: { prompt, style: state.style, archetypeId: state.type === 'image' ? null : state.archetypeId, imageStyleId: state.type === 'image' ? state.imageStyleId : null, attachment: state.attachment, baseVersionId: state.page === 'workspace' ? artifact()?.id : undefined, requestId: crypto.randomUUID() } };
  generation = token;
  state.busy = true;
  state.error = '';
  state.progress = '准备创作';
...
async function handleClick(e) {
  ...
  const actions = {
    ...
    copy: async () => { if (!artifact()) return; try { await navigator.clipboard.writeText(sourceFor(artifact())); toast('作品内容已复制。'); } catch { toast('浏览器未允许复制，可在源内容中选中复制，或直接导出。'); } },
