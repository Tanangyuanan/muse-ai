...
export async function createApp(config, options = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (!publicRequest && !/^(localhost|127\.0\.0\.1|\[::1\])(?::\d+)?$/.test(host)) throw new AppError('此工作空间不接受这个访问地址。', 403);
      ...
      let file;
      try { file = await realpath(join(webRoot, relative)); } catch { throw new AppError('文件不存在。', 404); }
